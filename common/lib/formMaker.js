(function(){
    ...
    angular.module("ones.formMaker", ["ones.select3Module"])
        ...
        //批量选择
        .directive("billBatchSelect", ["$injector", "$compile", function($injector, $compile){
        	return {
                restrict: "A",
                compile: function(element, attrs, transclude) {
                    return {
                    	pre: function($scope, iElement, iAttrs, controller) {
                    		var tpl = '<ul class="dropdown-menu batch-select-ul" id="%(batch_select_id)s" role="menu" aria-labelledby="dropdownMenu">' +
                    			'<li ng-repeat="s in %(var)s" ng-bind="s.%(name_field)s" ng-click="doBatchSelectClick(s, \'%(field)s\')"></li>' +
                    			'</ul>';
                    		
                    		var hideBatchSelect = function() {
                    			$('.batch-select-ul').hide();
                    		};
                    		...
                    		$scope.$parent.doBatchSelect = function(field){
                    			var structure = $scope.fieldsDefine[field];
                    			var name_field = structure.nameField || "name";
                    			var api;
                    			
                    			if(!structure.dataSource.api) {
                    				api = $injector.get(structure.dataSource);
                    			} else {
                    				api = structure.dataSource.api;
                    			}
                    			
                    			var scopeVarKey = 'batch_select_'+field;
                    			
                    			if($('#'+scopeVarKey).length) {
                    				$('#'+scopeVarKey).show();
                    				return;
                    			}
                    			
                    			if(typeof(api.api) == 'function') {
                    				api = api.api;
                    			}
                    			api.query(function(data){
                    				$scope[scopeVarKey] = data;
                    				tpl = sprintf(tpl, {
                    					batch_select_id: scopeVarKey,
                    					'var': scopeVarKey,
                    					name_field: name_field,
                    					field: field
                    				});
                    				angular.element("body").append($compile(tpl)($scope));
                    				setTimeout(function(){
                    					$('#'+scopeVarKey).css({
                    		...
                    		$scope.doBatchSelectClick = function(batchItem, field, name_field) {
                    			var struct = $scope.fieldsDefine[field];
                    			var name_field = struct.nameField || "name";
                    				...
                    				if(struct.editAbleRequire) {
                    					if(struct.editAbleRequire instanceof Array) {
                                            for(var i=0;i<struct.editAbleRequire.length;i++) {
                                            	if(!item[struct.editAbleRequire[i]]) {
                                            		return false;
                                            	}
                                            }
                                        } else {
                                        	if(!item[struct.editAbleRequire]) {
                                        		return false;
                                        	}
                                        }
                    				}
                    				$scope.billData[k][field] = batchItem.id,
                    				$scope.billData[k][field+"_label"] = batchItem[name_field]
                    			});
                    			hideBatchSelect();
                    		}
                    	}
...
//                            console.log(fieldDefine["ng-model"]);
                ...
                service.makeBill = function($scope, config){
                    ...
                    this.scope.$parent.fieldsDefine = this.opts.fieldsDefine;
                ...
                service.makeBill.prototype = {
                    ...
                    makeHead: function(fieldsDefine){
                        ...
                        angular.forEach(fieldsDefine, function(item, field){
                            ...
                            if(item.batchSelect == true) {
                            	item.displayName += ' <i class="fa fa-chevron-down" ng-click="doBatchSelect(\''+field+'\')"></i>';
                            	attr.push('bill-batch-select')
                            }
